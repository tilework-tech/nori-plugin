...
**Cursor IDE support** is provided by the `cursor/` subdirectory, which uses a two-tier registry architecture: CursorLoaderRegistry (@/src/cli/features/cursor/cursorLoaderRegistry.ts) for top-level loaders, and CursorProfileLoaderRegistry (@/src/cli/features/cursor/profiles/cursorProfileLoaderRegistry.ts) for profile-dependent loaders. This registry is used by the `install-cursor` command (@/src/cli/commands/install-cursor/installCursor.ts) and installs profiles, skills, and AGENTS.md to `~/.cursor/`. The cursor loaders reuse the same profile templates from @/src/cli/features/profiles/config/ (which now include AGENTS.md files for Cursor) but write to Cursor paths. See @/src/cli/features/cursor/docs.md for details.
