...
Executable hook scripts for conversation summarization, desktop notifications, package auto-updates, and instant profile switching. Contains five hook implementations that Claude Code invokes at lifecycle events: summarize.ts (memorizes conversations to backend), summarize-notification.ts (displays sync user notification), autoupdate.ts (checks and installs package updates), notify-hook.sh (cross-platform desktop notifications), and quick-switch.ts (intercepts profile switch commands for instant execution).
