...
**current** (@/src/cli/commands/current-skillset/currentSkillset.ts): Displays the currently active skillset name. The command has two forms: `current` (primary, shorthand) and `current-skillset` (hidden alias for backward compatibility). Uses `loadConfig()` to read the centralized config, then `getAgentProfile({ config, agentName })` to retrieve the active profile. Supports `--agent` option to specify which agent's skillset to display; defaults to the first installed agent when not specified. Outputs the skillset name via `raw()` logger for easy parsing in scripts. Exits with error if no config exists or no active skillset is configured.
